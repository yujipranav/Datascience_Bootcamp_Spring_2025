--1.Pull total number of orders that were completed on 18th March 2023 with the first name ‘John’ and last name Doe’.

SELECT COUNT(*) FROM SALES S
JOIN CUSTOMERS C ON S.CUSTOMER_ID=C.CUSTOMER_ID
WHERE C.FIRST_NAME='John' AND C.LAST_NAME='Doe';


--2.Pull total number of customers that purchased in 2023 and the average amount spend per customer.
SELECT C.CUSTOMER_ID,C.FIRST_NAME,C.LAST_NAME,AVG(REVENUE) AS AVG_COST
FROM CUSTOMERS C JOIN SALES S ON C.CUSTOMER_ID=S.CUSTOMER_ID
WHERE DATE BETWEEN '2023-01-01' AND '2023-12-31'
GROUP BY C.CUSTOMER_ID;

--3.Pull the departments that generated less than $800 in 2022.
SELECT I.DEPARTMENT FROM ITEMS I
JOIN SALES S ON S.ITEM_ID=I.ITEM_ID
WHERE DATE BETWEEN '2022-01-01' AND '2022-12-31'
GROUP BY I.DEPARTMENT
HAVING SUM(REVENUE)>800; 

--4.What is the most and least revenue we have generated by an order and sort it in Descending order
SELECT 'Max' AS which, MAX(revenue) AS revenue FROM sales UNION ALL SELECT 'Min' AS which, MIN(revenue) AS revenue FROM sales ORDER BY revenue DESC;

--5.What were the orders that were purchased in our most lucrative order.
SELECT * FROM SALES
WHERE REVENUE=(SELECT MAX(REVENUE) FROM SALES);
